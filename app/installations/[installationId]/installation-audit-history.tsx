...
import {
  ...
  type InstallationAuditListItem,
  type InstallationAuditPagePayload,
} from "@/lib/installations/installation-audit-presentation";

const PAGE_SIZE = 25;

function AuditEvent({ event }: Readonly<{ event: InstallationAuditListItem }>) {
  return (
    <li className="border-b border-stone-200 px-5 py-4 last:border-b-0">
      <article>
        <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
          <h3 className="text-sm font-semibold text-stone-950">
            {INSTALLATION_AUDIT_EVENT_LABELS[event.eventType]}
          </h3>
          <time
            className="shrink-0 text-sm text-stone-600"
            dateTime={event.occurredAt}
          >
            {formatInstallationAuditDateTime(event.occurredAt)}
          </time>
        </div>
        <dl className="mt-3 grid gap-x-8 gap-y-3 text-sm sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <dd className="mt-1 text-stone-900">
              {event.changedFields
                .map((field) => INSTALLATION_AUDIT_FIELD_LABELS[field])
                .join(", ")}
            </dd>
          </div>
        </dl>
...
export function InstallationAuditHistory({
  initialPage,
  installationId,
}: Readonly<{
  initialPage: InstallationAuditPagePayload;
  ...
  async function loadMore() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/installations/${encodeURIComponent(installationId)}/audit?${searchParams}`,
        {
          cache: "no-store",
      ...
      const page = parseInstallationAuditPage(
        await response.json(),
        installationId,
        events,
      );
  ...
  return (
    <section
      aria-labelledby="installation-audit-history-heading"
      className="rounded-md border border-stone-300 bg-white"
    >
      <div className="border-b border-stone-200 px-5 py-3">
        <h2
          className="text-base font-semibold text-stone-950"
          id="installation-audit-history-heading"
        >
          Händelsehistorik
        </h2>
        <p className="mt-1 text-sm text-stone-600">
          Registrerade ändringar visas nyast först som metadata utan tidigare
          eller nya fältvärden.
        </p>
      </div>

      {events.length === 0 ? (
        <p className="p-5 text-sm text-stone-600">
          Det finns inga registrerade händelser för denna installation.
        </p>
      ) : (
