...
select is((select count(*)::integer from pg_policies where schemaname='public' and tablename='licenses'),0,'licenses zero policies');
...
select is((select count(*)::integer from unnest(array['SELECT','INSERT','UPDATE','DELETE','TRUNCATE','REFERENCES','TRIGGER','MAINTAIN']) p where has_table_privilege('authenticated','public.licenses',p)),0,'authenticated no licenses privileges');
...
select is((select count(*)::integer from pg_class c cross join lateral aclexplode(c.relacl) a where c.oid='public.licenses'::regclass and a.grantee<>c.relowner),0,'licenses no nonowner ACL or grant options');
...
select is((select count(*)::integer from pg_policies where schemaname='public' and tablename='license_terms_versions'),0,'license_terms_versions zero policies');
...
select is((select count(*)::integer from unnest(array['SELECT','INSERT','UPDATE','DELETE','TRUNCATE','REFERENCES','TRIGGER','MAINTAIN']) p where has_table_privilege('authenticated','public.license_terms_versions',p)),0,'authenticated no license_terms_versions privileges');
...
select is((select count(*)::integer from pg_class c cross join lateral aclexplode(c.relacl) a where c.oid='public.license_terms_versions'::regclass and a.grantee<>c.relowner),0,'license_terms_versions no nonowner ACL or grant options');
...
select is((select count(*)::integer from pg_proc where pronamespace='public'::regnamespace and (proname like '%license%' or proname like '%licensing%')),0,'no Licensing functions including AAL2 and write RPC');
set local role anon;
...
select throws_ok($q$select * from public.licenses$q$,'42501',null,'authenticated denied select licenses');
...
select throws_ok($q$select * from public.license_terms_versions$q$,'42501',null,'authenticated denied select license_terms_versions');
