...
select set_eq($q$select proname::text from pg_proc where pronamespace='public'::regnamespace and (proname like '%license%' or proname like '%licensing%')$q$, $q$values ('is_licensing_owner_aal2')$q$, 'no other Licensing functions, including mutation or audit RPC');
