create function public.list_installation_audit_events(
...
begin
  ...
  if p_cursor_id is not null
    and not exists (
      select 1
      from public.installation_audit_events as cursor_event
      where cursor_event.installation_id = p_installation_id
  ...
  with candidate_events as materialized (
    select audit.*
    from public.installation_audit_events as audit
    where audit.installation_id = p_installation_id
      and (
        p_cursor_id is null
...
comment on function public.list_installation_audit_events(uuid, integer, timestamp with time zone, uuid) is
  'Lists one installation audit history newest-first with a typed, installation-bound cursor and no direct audit table grant.';

revoke execute on function public.list_installation_audit_events(uuid, integer, timestamp with time zone, uuid) from public;
revoke execute on function public.list_installation_audit_events(uuid, integer, timestamp with time zone, uuid) from anon;
revoke execute on function public.list_installation_audit_events(uuid, integer, timestamp with time zone, uuid) from service_role;
grant execute on function public.list_installation_audit_events(uuid, integer, timestamp with time zone, uuid) to authenticated;
