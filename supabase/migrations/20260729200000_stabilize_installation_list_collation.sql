create or replace function public.list_installations(
...
begin
  ...
  if p_cursor_id is not null
    and not exists (
      ...
      where cursor_installation.id = p_cursor_id
        and cursor_installation.display_name collate "C" =
          p_cursor_display_name collate "C"
  ...
  with filtered_installations as materialized (
    ...
    where (p_tenant_id is null or installation.tenant_id = p_tenant_id)
      ...
      and (
        p_cursor_id is null
        or (
          installation.display_name collate "C",
          installation.id
        ) > (
          p_cursor_display_name collate "C",
          p_cursor_id
    ...
    order by installation.display_name collate "C" asc, installation.id asc
  ...
  numbered_installations as (
    select
      candidate.*,
      row_number() over (
        order by candidate.display_name collate "C" asc, candidate.id asc
      ) as page_position
  ...
  page_metadata as (
    select
      ...
      max(numbered.display_name collate "C")
        filter (
  ...
  order by page.display_name collate "C" asc, page.id asc;
end;
$function$;

comment on function public.list_installations(integer, text, uuid, uuid, text, text, boolean, text) is
  'Lists owner-visible installations with safe metadata, typed filters, literal search, and stable C-collated display-name/UUID cursor pagination.';
